...
autonomous: true
requirements: [SAVE-01, SAVE-02 (partial), SAVE-03 (partial), SAVE-04, SAVE-05]
# SAVE-02 partial: Only run-end auto-save implemented here. Tech-tree-purchase save trigger deferred to Phase 6.
# SAVE-03 partial: Placeholder fields for TechTreeUnlocks and PlayerStats included but not populated until Phase 6.

must_haves:
  truths:
    - "Game state saves to a JSON file on run end (GameOver transition); tech-tree-purchase save trigger deferred to Phase 6"
    - "Save file contains credits, current level, version number, and placeholder fields for tech tree unlocks and player stats"
  ...
  <action>
    **SaveData.cs** -- Plain C# class (NOT a MonoBehaviour):
    ...
    - Fields:
      ...
      - `public bool[] TechTreeUnlocks = new bool[0];` -- PLACEHOLDER for Phase 6 tech tree state (SAVE-03). Empty array now; Phase 6 will size it to match the tech tree node count. Additive — no migration needed.
      - `[System.Serializable] public class PlayerStatsData { public float MiningRadius = 1f; public float DamageMultiplier = 1f; }` -- Nested serializable class. Define INSIDE SaveData.cs but outside SaveData class (or as a nested class).
      - `public PlayerStatsData Stats = new PlayerStatsData();` -- PLACEHOLDER for Phase 6 player stats persistence (SAVE-03). Defaults match base stats. Phase 6 will read/write these after tech tree purchases.
    ...
    **SaveManager.cs** -- MonoBehaviour (singleton pattern):
    ...
    - Self-instantiation via `[RuntimeInitializeOnLoadMethod(RuntimeInitializeLoadType.BeforeSceneLoad)]` static method:
      - `private static void EnsureInstance()`: if `Instance == null`, create a new GameObject("SaveManager"), add SaveManager component, call `DontDestroyOnLoad`. This eliminates the need for manual scene setup — SaveManager is always available regardless of scene configuration. The null-conditional `SaveManager.Instance?.AutoSave()` pattern in consumers is still safe but will no longer silently skip saves.
    - `Awake()`: singleton guard (same pattern as GameManager — if Instance exists and isn't this, destroy self). Set `_currentSave = Load();`.
    ...
    **PlayingState.cs** -- Add save load on first run:
    ...
    NOTE: SaveManager self-instantiates via [RuntimeInitializeOnLoadMethod] — no manual scene setup required. It creates its own GameObject and calls DontDestroyOnLoad. No CoPlay MCP or manual scene addition needed.
  </action>
  <verify>
    All 4 files exist. Grep confirms:
    - `SaveData.cs` contains `SaveVersion`, `TotalCredits`, `CurrentLevel`, `TechTreeUnlocks`, `PlayerStatsData`, `[System.Serializable]`
...
<verification>
- SaveData has SaveVersion = 1, TotalCredits (long), CurrentLevel (int), TechTreeUnlocks (bool[]), Stats (PlayerStatsData)
- SaveManager self-instantiates via [RuntimeInitializeOnLoadMethod] — no scene setup required
