...
must_haves:
  ...
  artifacts:
    - path: "Assets/Scripts/ECS/Components/MineralComponents.cs"
      provides: "MineralTag, MineralData, MineralPullData, MineralsSpawnedTag, CollectionEvent"
...
<task type="auto">
  ...
  <action>
    **MineralComponents.cs** -- Create 5 unmanaged IComponentData structs plus 1 IBufferElementData:
    ...
    - `MineralsSpawnedTag : IComponentData` -- empty struct, added to asteroids after minerals are spawned to prevent double-spawn (see MineralSpawnSystem)
    ...
    **MineralSpawnSystem.cs** -- Burst-compiled ISystem:
    ...
    - Iterate `SystemAPI.Query<RefRO<HealthData>, RefRO<LocalTransform>>().WithAll<AsteroidTag>().WithNone<MineralsSpawnedTag>().WithEntityAccess()`.
    - The `.WithNone<MineralsSpawnedTag>()` filter prevents double-spawn: the asteroid entity persists for one extra frame while ECB destruction queues, so without this guard the same dead asteroid would trigger mineral spawning twice.
    - When `CurrentHP <= 0f`: spawn minerals AND add `MineralsSpawnedTag` to the asteroid entity via ECB (`ecb.AddComponent<MineralsSpawnedTag>(sortKey, asteroidEntity)`). Spawn `_rng.NextInt(GameConstants.MinMineralsPerAsteroid, GameConstants.MaxMineralsPerAsteroid + 1)` mineral entities.
  ...
  <verify>
    ...
    - `MineralComponents.cs` contains `MineralTag`, `MineralData`, `MineralPullData`, `MineralsSpawnedTag`, `CollectionEvent`
    - `MineralSpawnSystem.cs` contains `UpdateBefore(typeof(AsteroidDestructionSystem))`, `WithNone<MineralsSpawnedTag>`, and `AddComponent<MineralsSpawnedTag>`
...
- MineralSpawnSystem uses WithNone<MineralsSpawnedTag> and adds MineralsSpawnedTag to prevent double-spawn
