...
- In DrainSkillEvents:
  ...
  buffer.Clear();
  ```
- Also extend DrainDamageEvents for critical hits:
  - **REMOVE the existing `CameraShake.Instance?.Shake()` call** inside the `if (evt.Type == DamageType.Critical)` block. Per user decision: crits have "no extra screen flash or shake". The current Phase 4 code fires CameraShake on all DamageType.Critical events which must be deleted.
  - **ADD** crit sound: When `evt.Type == DamageType.Critical`, call `AudioManager.Instance?.PlayCritHit(pos);` (sound only, no shake).
  - The resulting code for the critical hit branch should be:
  ```csharp
...
6. Watch for "CRIT!" popups: While mining normally, some damage numbers should occasionally appear as yellow "CRIT!" text (~8% of hits) with a distinct crit sound. Confirm there is NO extra screen shake on crits (per user decision).
...
8. **Crit popups**: Yellow "CRIT!" at ~8% rate with distinct crit sound. No extra screen shake on crits (per user decision).
