...
autonomous: true
requirements: [SKIL-01, SKIL-02, SKIL-03, SKIL-04, SKIL-05, DMGS-01, DMGS-02]
...
must_haves:
  truths:
    ...
    - "Asteroid stops taking periodic damage ticks 2-3 seconds after being hit by Laser Burst or EMP Pulse, and ember particles cease"
  artifacts:
  ...
  key_links:
    - from: "Assets/Scripts/MonoBehaviours/Bridge/InputBridge.cs"
      ...
      via: "Keyboard.current digit keys OR into existing SkillInputData each frame (preserves UI button presses)"
      pattern: "skillInput\\.Skill[1-4]Pressed \\|="
...
**NOTE:** The ECS skill systems that consume SkillInputData must clear the flags after reading them (set all to false after processing). This prevents a single press from firing on consecutive frames. Add this reset at the end of SkillCooldownSystem.OnUpdate or in a dedicated system that runs after all skill systems.
...
<task type="auto">
  <name>Task 2a: Implement SkillCooldownSystem, OverchargeSystem, and crit integration in MiningDamageSystem</name>
  <files>
    Assets/Scripts/ECS/Systems/SkillCooldownSystem.cs
    Assets/Scripts/ECS/Systems/OverchargeSystem.cs
    Assets/Scripts/ECS/Systems/MiningDamageSystem.cs
...
- **IMPORTANT: At the END of OnUpdate, after cooldown decrements, reset SkillInputData to all-false.** This ensures that skill press flags set by InputBridge or SkillBarController are consumed exactly once per frame and do not persist into the next frame. Read the singleton RW and set all `SkillNPressed = false`.

**2. Create `OverchargeSystem.cs`:**
...
**3. Modify `MiningDamageSystem.cs`:**
...
1. Press 4 -- Overcharge makes mining circle deal more damage (check MiningDamageSystem output)
2. Mining circle damage occasionally crits (yellow CRIT! popups appear since FeedbackEventBridge already handles DamageType.Critical)
3. Skills respect cooldowns (pressing key during cooldown does nothing)
  </verify>
  <done>
SkillCooldownSystem decrements cooldowns and resets SkillInputData flags each frame. OverchargeSystem activates buff on key press. MiningDamageSystem integrates crit rolls and Overcharge buff multipliers. Zero compilation errors.
  </done>
</task>

<task type="auto">
  <name>Task 2b: Implement LaserBurstSystem, ChainLightningSystem, EmpPulseSystem, and BurningDamageSystem</name>
  <files>
    Assets/Scripts/ECS/Systems/LaserBurstSystem.cs
    Assets/Scripts/ECS/Systems/ChainLightningSystem.cs
    Assets/Scripts/ECS/Systems/EmpPulseSystem.cs
  ...
  <action>
**CRITICAL RULES (same as Task 2a):**
- `[BurstCompile]` on struct and OnUpdate ONLY -- NEVER on OnCreate
- All systems must guard `gameState.Phase != GamePhase.Playing` at top of OnUpdate
- Use `Unity.Mathematics.Random` for crit rolls (seed in OnCreate from `(uint)System.Environment.TickCount`)
- Use `SystemAPI.GetSingletonBuffer<DamageEvent>()` and `SystemAPI.GetSingletonBuffer<SkillEvent>()` for event emission
...
**2. Create `ChainLightningSystem.cs`:**
...
**4. Create `BurningDamageSystem.cs`:**
...
4. Asteroids hit by Laser/EMP have BurningData component (verify in Entity Debugger) and take periodic damage
5. Skills respect cooldowns (pressing key during cooldown does nothing)
6. DoT popups from BurningDamageSystem appear white (DamageType.Normal), not orange. Confirm DamageType.DoT is not used.
  </verify>
  <done>
All three offensive skill systems deal damage with correct targeting (line/chain/AoE). BurningDamageSystem ticks DoT on burning asteroids using DamageType.Normal (white popups) and removes BurningData on expiry. SkillEvent buffer populated for Plan 02 VFX consumption. Cooldowns enforced.
  </done>
</task>
...
7. **Critical hits**: Yellow "CRIT!" popups appear at ~8% rate on mining ticks and skill hits (no extra screen flash or shake per user decision).
